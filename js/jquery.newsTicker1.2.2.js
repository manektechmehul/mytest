...
(function($) {
	...
	function runTicker(settings) {
		...
		else if ($(settings.newsList).data('over').state == 1) {
			//$(settings.newsList).data('over', {state: 0})
			currentLength = settings.newsItems[currentItem].length + 2;
		}
		...
		if( currentLength <= settings.newsItems[currentItem].length + 1) {
			...
			if ($(settings.newsList).data('over').state == 0) {
				currentLength = 0;
				currentItem ++;
			}
	...
	$.fn.extend({
		newsTicker: function(settings) {
			...
			$(settings.newsList + ' li a').live('mouseover mouseout', function(event) {
				if (event.type == 'mouseover')
					$(this).parent().parent().data('over', {state: 1});
				else
					$(this).parent().parent().data('over', {state: 0});
			});
