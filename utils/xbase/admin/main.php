...
class main extends template {
    ...
    function main() {
        ...
        /* #module specific */
        $this->group_name = '[[PLURALUPPER]]';
        /* #module specific */
        $this->single_name = '[[UPPERCASE_NAME]]';
        $this->ordered = true;
        ...
        // this will populate the link table - categories
         /* #module specific */
        $this->links = array('category' => array('link_table' => '[[CATEGORY_LOOKUP_TABLE]]', 'table' => '[[CATEGORY_TABLE]]', 'name' => 'title', 'primary'=>'item_id', 'foreign'=>'category_id'));
    }
	...
	function onload() {
		...
		if ( [[ALLUPPER]]_HAS_INLINE_GALLERIES ) {
    ...
    function get_form_data() {
        $this->module_id = db_get_single_value("select id from module where constant = 'SITE_HAS_[[ALLUPPER]]'", "id");
   ...
   function category_checklist($id, $fieldname, $field) {
        ...
        /* #module specific */
        $linksql = "SELECT t.id, t.title, l.item_id FROM [[CATEGORY_TABLE]] t 
LEFT OUTER JOIN [[CATEGORY_LOOKUP_TABLE]] l ON t.id = l.category_id AND l.item_id = '$id' order by title";      
...
/* #module specific */
$second_admin_tab = "[[LOWERCASE_NAME]]";
