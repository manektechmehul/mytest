...
class [[UPPERCASE_NAME]]Module extends PageListModule {
    ...
    function __construct($module_path, $paginate, $paginate_items_per_page, $pageUrl, $module_key) {
        ...
        $this->listName = '[[PLURALLOWER]]';
        $this->itemName = '[[LOWERCASE_NAME]]';
        $this->pluralName = '[[PLURALUPPER]]';
        $this->singleName = '[[UPPERCASE_NAME]]';
        $this->section_page_name = '[[UPPERCASE_NAME]]';

        // technical names
        $this->table = '[[UPPERCASE_NAME]]';
        $this->categoryTable = '[[CATEGORY_TABLE]]';
        $this->linkTable = '[[CATEGORY_LOOKUP_TABLE]]';
        $this->linkField = 'category_id';
        $this->keyField = 'item_id';

        $this->sideboxListName = 'sidebox[[PLURALLOWER]]'; // ?? odd name
        $this->pageName = $pageName;
        $this->currentCategory = "";

        /* #module specific */
        $this->model = new [[LOWERCASE_NAME]]model($pageUrl);
        $this->model->setTable('[[LOWERCASE_NAME]]');

        $this->model->setpublishedField('published');
        $this->model->setOrderField('order_num desc');

        /* #module specific */
        $this->sidebox_filter_key = "[[LOWERCASE_NAME]]";
...
WHERE page_type.config_flag = '" . $module_key . "';";
    ...
    function createSearchBox($hasCategories) {
        ...
        $filters[$this->itemName . '_[[LOWERCASE_NAME]]_search_filter'] = array('search_string' => '/<!-- CS ' . $this->sidebox_filter_key . ' searchbox start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' searchbox end -->/s',
            'replace_string' => '{include file="file:' . $template_file . '"}');
    }
