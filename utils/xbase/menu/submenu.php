...
if([[ALLUPPER]]_ON_LEVEL == 0){
    ...
    /* #module specific */
    $submenu = get[[PLURALUPPER]]Submenu($menulink);
...
/* #module specific */
function get[[PLURALUPPER]]Submenu($menulink) {
    // change trigger
    /* #module specific */
  if([[ALLUPPER]]_HAS_CATEGORIES != 0){
	...
	$pageMenu = array();	
        ...
        /* #module specific */
		$subnav_sql="SELECT DISTINCT c.* FROM [[CATEGORY_LOOKUP_TABLE]] csc
INNER JOIN [[MAIN_TABLE]] cs ON cs.id = csc.item_id
INNER JOIN [[CATEGORY_TABLE]] c ON c.id = csc.category_id
