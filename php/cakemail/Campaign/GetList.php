 ...
 */
	...
	try {
		...
		$xml->init();
		$xml->add_class("ClassCampaign", $locale);
	...
	if (isset($r[0]["class"][0]["method"])) {
		...
		foreach ($r[0]["class"][0]["method"] as $result) {
			...
			case "count":
				...
				break;
			case "campaign":
				foreach ($result["campaign"] as $element) {
					$res["campaigns"][$i][$element["name"]] = $element["value"];
				}
				$i++;
			}
		}
	...
	if (isset($res["count"]) && isset($res["campaigns"])) {
		unset($res["campaigns"]);
	}
