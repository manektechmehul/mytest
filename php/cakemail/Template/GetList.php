 ...
 */
	...
	try {
		...
		$xml->init();
		$xml->add_class("ClassTemplate", $locale);
	...
	if (isset($r[0]["class"][0]["method"])) {
		...
		foreach ($r[0]["class"][0]["method"] as $result) {
			...
			case "error_message":
				...
				break;
			case "template":
				foreach ($result["template"] as $element) {
					$res["templates"][$i][$element["name"]] = $element["value"];
				}
				$i++;
				break;
			default:
	...
	if (isset($res["count"]) && isset($res["templates"])) {
		unset($res["templates"]);
	}
