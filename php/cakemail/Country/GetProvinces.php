 ...
 */
	...
	try {
		...
		$xml->init();
		$xml->add_class("ClassCountry", $locale);
		$xml->add_method("GetProvinces");
	...
	if (isset($r[0]["class"][0]["method"])) {
		...
		foreach ($r[0]["class"][0]["method"] as $result) {
			...
			case "error_message":
				...
				break;
			case "province":
				foreach ($result["province"] as $element) {
					$res["provinces"][$i][$element["name"]] = $element["value"];
				}
				$i++;
			}
		}
