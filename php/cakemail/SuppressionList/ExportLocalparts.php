...
/**
 * SuppressionList/ExportLocalparts.php
 *
 ...
 */
	...
	try {
		...
		$xml->add_method("ExportLocalparts");
		...
		foreach ($r[0]["class"][0]["method"] as $result) {
			...
			case "record":
				foreach ($result["record"] as $element) {
					switch ($element["name"]) {
					case "localpart":
						$res["localparts"][$i]["localpart"] = $element["value"];
						break;
					}
				}
				$i++;
	...
	if (isset($res["count"]) && isset($res["localparts"])) {
		unset($res["localparts"]);
	}
