...
class imageupload_field extends abstract_field {
	...
	function isDataInvalid( $key = '' ) {
		/*
		 * will set $_SESSION['last_image_upload_location'] for use in the auto_form.php to write into your db
		 * if you set $_SESSION['form_image_upload_cache_name'] before you arrive - then thats where the function will save the image
		...
		umask( 0 );
		$file             = $this->data;
		$valid_extensions = explode( ',', PERMITTED_UPLOAD_IMAGE_TYPES );
		$upload_dir       = 'UserFiles/Image/public_uploads';
		$db_friendly_dir  = 'public_uploads';
		...
		do {
			if ( $key ) {
				...
				$web_relative_address                   = '/' . $db_friendly_dir . '/' . $keyname . '.' . $ext;
		...
		if ( ! in_array( $ext, $valid_extensions ) ) {
			...
			$uploadErrors = array(
				'1' => 'File too large',
				'2' => 'File too large',
				'3' => 'Interupted file upload',
				'4' => 'No File',
				'6' => 'Contact Site Administrator',
				'7' => 'Contact Site Administrator',
				'8' => 'Invalid file type'
			);
