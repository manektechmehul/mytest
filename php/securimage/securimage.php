...
class securimage {
  ...
  var $save_data_directory = "./image_data";
  var $check_data_directory = "./php/securimage/image_data";
  ...
  var $hash_salt = "DpJFTlmlsjgWcQUZ8";
  ...
  function saveData()
  {
    ...
    $fp = fopen($this->save_data_directory . "/" . $filename, "w+");
  ...
  function validate()
  {
    ...
    $enced_code = trim(@file_get_contents($this->check_data_directory . "/" . $filename));
    
    $check = md5($this->hash_salt . strtolower($this->code_entered));

    if($check == $enced_code) {
      $this->correct_code = TRUE;
      @unlink($this->check_data_directory . "/" . $filename);
    } else {
  ...
  function pruneOld()
  {
    if ($handle = @opendir($this->check_data_directory)) {
      while (($filename = readdir($handle)) !== false) {
        if(time() - filemtime($this->check_data_directory . "/" . $filename) > $this->prune_minimum_age * 60) {
          @unlink($this->check_data_directory . "/" . $filename);
        } 
      }
      closedir($handle);
