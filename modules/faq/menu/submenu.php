...
if(FAQ_ON_LEVEL == 0){
    ...
    /* #module specific */
    $submenu = getFaqSubmenu($menulink);
...
/* #module specific */
function getFaqSubmenu($menulink) {
    // change trigger
    /* #module specific */
  if(FAQ_HAS_CATEGORIES != 0){
	...
	$pageMenu = array();	
        ...
        /* #module specific */
		$subnav_sql="SELECT DISTINCT c.* FROM faq_category_lookup csc
INNER JOIN faq cs ON cs.id = csc.item_id
INNER JOIN faq_category c ON c.id = csc.category_id
