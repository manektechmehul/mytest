...
class members {
    ...
    static function GetDetailsByUsername($username) {
        $sql = "select * from member_user where status = 1 and username = '$username'";
        return db_get_single_row($sql);
    }
    
    static function GetDetailsByEmail($email) {
        $sql = "select * from member_user where status = 1 and email = '$email'";
        return db_get_single_row($sql);
    }
    
    static function CheckIfUserWithEmailExists($email) {
        $sql = "select count(*) from member_user where email = '$email'";
        return  db_get_single_value($sql) > 0;
    }    
    
    static function CheckIfUserWithUsernameExists($username) {
        $sql = "select count(*) from member_user where username = '$username'";
        return  db_get_single_value($sql) > 0;
    }        

    static function CheckIfUserWithScreenNameExists($screenName) {
        $sql = "select count(*) from member_user where screenname = '$screenName'";
        return  db_get_single_value($sql) > 0;
    }        
    
    static function CheckUsernameResetCode($username, $code) {
        $sql = "select count(*) from member_user where status = 1 and username = '$username' and resetcode = '$code'";
        $count = db_get_single_value($sql);
        return $count > 0;
    }

    static function CheckEmailResetCode($email, $code) {
        $sql = "select count(*) from member_user where status = 1 and email = '$email' and resetcode = '$code'";
        $count = db_get_single_value($sql);
        return $count > 0;
    }
    ...
    static function ResetPasswordByUsername($username, $code, $password) {
        ...
        $sql = "update member_user set password = '$hash', resetcode = '' where status = 1 and username = '$username' and resetcode = '$code'";
        mysql_query($sql);
    }

    static function ResetPasswordByEmail($email, $code, $password) {
        $hash = self::GetNewPasswordHash($password);
        $sql = "update member_user set password = '$hash', resetcode = '' where status = 1 and email = '$email' and resetcode = '$code'";
        mysql_query($sql);
    }
    ...
    static function SetupResetCode($userId) {
        $resetCode = md5(time());
        $sql = "update member_user set resetcode = '$resetCode' where status = 1 and id = '$userId'";
        mysql_query($sql);
        return $resetCode;
    }
    ...
    static function CheckCurrentPassword($password) {
        ...
        $hash = db_get_single_value('select password from member_user where id = '.$id);
        $pass = new PasswordHelper();
    ...
    static function SetPassword ($password) {
        ...
        $sql = "update member_user set password = '$hash' where id = $id";
