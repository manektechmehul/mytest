...
// set this class to unique name
// inpagelinksModule
// based on docsModule
class inpagelinks_Module extends listModule 
{
	public $listName;
	...
	function __construct($module_path, $section_id)
	{
		$this->listName = '';
		$this->itemName = '';
		$this->sideboxListName = '';
		$this->pluralName = '';
		$this->singleName = '';
		$this->section_page_name = '';
		...
		$this->keyField = '';
                ...
                /* 
                 * Other chnages to make if duplication as a standard module
                 * 
                 * function AssignSearchCategories 
                 * - change smartie tag name + replace in the search tpl
                 * 
                 * 
                 */                             
	...
	function GetItems()
	{
		...
		$sql = "select SQL_CALC_FOUND_ROWS t.id, title, description, thumb,  summary, file, link, ".
        ...
        /* Get all items - typically for  alist view */
	function GetAllItems($limit='', $not_in="")
	{
		...
		$sql = "select SQL_CALC_FOUND_ROWS t.id, title, thumb, description, summary, file, unix_timestamp(`date`) as `date` from {$this->table} t ".
			$cat_join .
			"where published = 1 ". $not_in . 
			$cat_clause .
			//"and deadline > now() ".
	...
	function GetSearchResultItems($searchKeywords, $category)
	{
        ...
        /* Get all the available categories to populate the drop down list in the side search bar */
	function GetSearchCategories()
	{
        ...
        /* Get the details of your item in the single item view */
	function GetSingleItem($id)
	{
		...
		if (mysql_num_rows($result) > 0) {	
			...
			$item['date'] = $row['date'];
                        ...
                        $item['file'] = '/UserFiles/File/' . $row['file'];
                        $item['file_name'] = $row['file_name'];
                        $item['file2'] = '/UserFiles/File/' . $row['file2'];
                        $item['file2_name'] = $row['file2_name'];
                        
                        $item['file3'] = '/UserFiles/File/' . $row['file3'];
                        $item['file3_name'] = $row['file3_name'];
                        $item['file4'] = '/UserFiles/File/' . $row['file4'];
                        $item['file4_name'] = $row['file4_name'];
                        $item['file5'] = '/UserFiles/File/' . $row['file5'];
                        $item['file5_name'] = $row['file5_name'];
                        $item['file6'] = '/UserFiles/File/'. $row['file6'];
                        $item['file6_name'] = $row['file6_name'];
                        $item['file7'] = '/UserFiles/File/'. $row['file7'];
                        $item['file7_name'] = $row['file7_name'];
                        $item['file8'] = '/UserFiles/File/'. $row['file8'];
                        $item['file8_name'] = $row['file8_name'];
	...
	function AssignSearchCategories($smartyTag)
	{
            /* updated to reciee a var */
		global $smarty;
		$searchCats = $this->GetSearchCategories();
		$smarty->assign($smartyTag, $searchCats);
	}

        
       
        
	function DisplayAll($limit='',$show_archive_link=false, $not_in='')
	{
		global $smarty;
		global $currentPageUrl;

		$items = $this->GetAllItems($limit, $not_in);

		$smarty->assign($this->listName, $items);
                //$paginationStr = $this->pagination->html_string($currentPageUrl.'?page=');
		//$smarty->assign('pagination', $paginationStr);
                if($show_archive_link){
                    $smarty->assign('show_archive_link','true');
                }
	...
	function SetCategoryForPage($pagename)
	{
        ...
        /* When in category list view - look up category title from the category id */
