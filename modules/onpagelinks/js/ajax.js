...
function getOPLData( rec_start, rec_no, search){
    ...
    var request = $.ajax({
        url: '/modules/onpagelinks/ajax.php',
        data: data,
        async: true,
        dataType: 'json',
        success: function (j) {
...
function createlink(obj){
  ...
  if(obj.video_type == "1"){
                out = out + "<div class='downloadcontent'  style='background-size: 120px Auto; background-position: -20px 0px;  background-image:url(http://img.youtube.com/vi/" + obj.video_id  + "/1.jpg);  '   >";
  }else{
       out = out + "<div class='downloadcontent'    >";
  }
