...
{assign var="alt" value="alt=`$casestudies_listtype`_`$casestudies_listName`"}
    {show_thumb_minimal filename=$casestudies_featured[item].thumb size=600x600}
        </div>
...
{assign var="listtype" value="Featured"}
{section loop=$casestudies_featured name=item }
                    <div class="img-block" style="background-image:url({show_thumb_minimal filename=$casestudies_featured[item].thumb size=600x600});"><a href="/{$casestudies_module_web_path}/{$casestudies_featured[item].page_name}" title="{$casestudies_featured[item].title}"></a></div>
                    <div class="img-info">
                      <div class="title">{$casestudies_featured[item].title}</div>
                      <p>{$casestudies_featured[item].description}</p>
                      <div class="link-group"> <a href="/{$casestudies_module_web_path}/{$casestudies_featured[item].page_name}" title="Read Case Study">Read Case Study</a> <a href="/{$casestudies_module_web_path}" title="Read more Case Studies">Read more Case Studies</a></div>
                    </div>
{/section}