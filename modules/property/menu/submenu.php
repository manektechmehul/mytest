...
if(PROPERTY_ON_LEVEL == 0){
    ...
    /* #module specific */
    $submenu = getPropertiesSubmenu($menulink);
...
/* #module specific */
function getPropertiesSubmenu($menulink) {
    // change trigger
    /* #module specific */
    
  if(PROPERTY_HAS_CATEGORIES != 0){
	...
	$pageMenu = array();	
        ...
        /* #module specific */
		$subnav_sql="SELECT DISTINCT c.* FROM property_category_lookup csc
INNER JOIN property cs ON cs.id = csc.item_id
INNER JOIN property_category c ON c.id = csc.category_id
