...
function gui_addArea(id) {
	...
	temp+= 'Coords: <input type="text" name="img_coords[]" class="img_coords" value="">';
        temp+= ' <input type="hidden" name="img_title" class="img_title" value="">';
      //  temp+= 'Image: <input type="file" name="img_images[]" class="img_img" value="">';
	temp+= ' <input type="text" name="img_href" class="img_href" value="">';
        //temp+= '<input type="hidden" name="img_href" class="img_href" />';
	temp+= ' <input type="text" name="img_alt" class="img_alt" value="">';        
	temp+= ' <select name="img_target" class="img_target">';
	...
	temp+= '</select>';
       // temp+= '<input type=button class="mapsave" value="Save" valatt="img_area_'+id+'" />';
      //  var valattr="img_area_"+id;
      //  temp+= '<a href="javascript:void(0);" onClick=checkTemp("'+valattr+'"); >Save</a>';
...
function gui_loadImage(src) {
	//reset zoom dropdown
       ...
       document.getElementsByName("floorplan_image")[0].value = src ; 
       var temp=src.split("floorplan/");
       src="http://localhost/UserFiles/floorplan/"+temp[1];
...
function gui_outputChanged() {
	...
	if (output == 'css') {
		...
		if (clipboard_enabled) {
			...
			temp+= '<img src="images/clipboard.gif" onclick="gui_toClipBoard()" style="float: right; margin: 4px; cursor: pointer;"/>';
		}
...
props = [];
//imgroot = 'C:/wamp/www/UserFiles/Image/';
//imgroot = 'http://localhost/UserFiles/Image/';
//imgroot = 'http://localhost/UserFiles/';
imgroot = 'UserFiles/';
outputmode = 'imgmap';
gui_outputChanged();
