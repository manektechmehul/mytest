<?php
	...
	class orders extends template {
		function orders() {
			...
			$this->group_name      = 'Booking Event Product Sales';
			$this->single_name     = 'Booking Event Product Sale';
			$this->singular        = 'an';
			$this->hideable        = true;
			$this->order_clause    = 'datestamp desc';
			$this->max_items       = - 1;
			$this->javascript_file = '/modules/booking/admin/js/orders.js';
			...
			$this->buttons = array(
				...
				'items' => array(
					...
					'type' => 'button',
					'pattern' => '/modules/booking/admin/order_items.php?order=%s'
				),
				'progress_order' => array(
					'text' => 'change status',
					'type' => 'button',
					'pattern' => '/modules/booking/admin/orders.php?change_status=%s'
				),
				'resend_email' => array(
					'text' => 'resend email',
					'type' => 'jsbutton',
		...
		function set_customer_id( $id, $row ) {
			...
			$href        = '/modules/booking/admin/orders.php?edit_customer=' . $customer_id;
		...
		function get_customer_filter_bar_custom_sql() {
			$where = " where soi.`product_type` = 2   "; // only every show the bookings entries
			if ( $_GET['donation_type'] != '-1' && $_GET['donation_type'] != '' ) {
				$where .= " and soi.product_id =" . $_GET['donation_type'] . " ";
			}
			if($_GET['status'] != '-1'){
				if($_GET['status']!=''){
