<?php
	...
	class BookingModule extends PageListModule {
		public $listName;
		...
		function __construct( $module_path, $paginate, $paginate_items_per_page, $pageUrl, $module_key ) {
			...
			$this->listName          = 'bookings';
			$this->itemName          = 'booking';
			$this->pluralName        = 'Bookings';
			$this->singleName        = 'Booking';
			$this->section_page_name = 'Booking';
			// technical names
			$this->table         = 'booking';
			$this->categoryTable = 'booking_category';
			$this->linkTable     = 'booking_category_lookup';
			$this->linkField     = 'category_id';
			$this->keyField      = 'item_id';
			$this->sideboxListName = 'sideboxbookings'; // ?? odd name
			$this->pageName        = $pageName;
			$this->currentCategory = "";
			/* #module specific */
			$this->model = new bookingmodel( $pageUrl );
			$this->model->setTable( 'booking' );
			$this->model->setpublishedField( 'published' );
			$this->model->setOrderField( 'order_num desc' );
			/* #module specific */
			$this->sidebox_filter_key = "booking";
...
WHERE page_type.config_flag = '" . $module_key . "';";
		...
		function DisplayItem( $id ) {
			...
			$item['category_name'] =  db_get_single_value('SELECT title FROM booking_category WHERE id = ' . $item['category_id']);


			if ( $item['hospice_event'] == '0' && $item['event_type'] != '3' ) // get tickets
			{
				$tickets = $this->getTickets( $item['id'] );
			...
			// to push case study image into page_image function in header
			global $filters;
			global $promo_image;
			if ( $item["page_image"] == "" ) {
				$promo_image = "/php/thumbimage.php?img=/UserFiles/Image/" . $item["thumb"] . "&size=50";
			} else {
				$promo_image = "/php/thumbimage.php?img=/UserFiles/Image/" . $item["page_image"] . "&size=50";
			}
		...
		function getTickets($id){
			$sql = "select * from booking_ticket where published=1 and booking_id=" . $id . " order by order_num";
			return db_get_rows($sql);

		}
		function getProducts($id){
			$sql = "select * from booking_product where published=1 and booking_id=" . $id . " order by order_num";
			return db_get_rows($sql);
		}
		...
		function ReadItems( $sql ) {
			...
			while ( $row = mysql_fetch_array( $result ) ) {
				...
				$items[] = array(
					...
					'speaker_info'  => $row['speaker_info'],
					'hospice_event' => $row['hospice_event'],
					'event_time'    => $row['event_time'],
					'event_type'    => $row['event_type'],
					'category_id'   => $row['category_id'],
					'category_name' => db_get_single_value('SELECT title FROM booking_category WHERE id = ' . $row['category_id'])
				);
			};
			return $items;
		}
		...
		function GetItems() {
			...
			/*	$sql = "SELECT SQL_CALC_FOUND_ROWS * " .
				       " from {$this->table} t " .
				       "where published = 1 " .
				       " AND ((end_date > NOW()) OR ((removal_date > NOW() AND persist_with_removal_date =1))) " .
				       "order by `order_num` desc " . $limits;
*/
				// in the main list of events don't show the expired items or the featured ones - these will be collected using a sidebox and add to the template
				$sql = "SELECT SQL_CALC_FOUND_ROWS * " .
				       " from {$this->table} t " .
				       "where published = 1 and featured= 0 " .
				       " AND end_date > NOW() - INTERVAL 1 DAY  " .
				       "order by end_date  " . $limits;
				...
				$sql = "SELECT SQL_CALC_FOUND_ROWS * " .
				       ...
				       "where published = 1 " .
				       $cat_clause .
				       " AND ((end_date > NOW() - INTERVAL 1 DAY) OR ((removal_date > NOW() AND persist_with_removal_date =1))) " .
				       "order by `order_num` desc " . $limits;
			}
		...
		function getLatestSidebox() {
			...
			$latest        = db_get_rows( "select * from {$this->table} where published = 1 and featured = 0 and end_date > NOW() - INTERVAL 1 DAY order by end_date   limit " . constant( $module_prefix . "LATEST_SIDEBOX_NUMBER_OF_ITEMS" ) );
		...
		function getFeaturedSidebox() {
			...
			global $module_prefix;
			$sql = "select * from {$this->table} ";
			$sql .= " where published = 1 and featured =1 and  end_date > NOW() - INTERVAL 1 DAY ";
			/*  if ($this->has_categories) {
		...
		function createSearchBox( $hasCategories=false ) {
			...
			$filters[$this->itemName . '_booking_search_filter'] = array( 'search_string'  => '/<!-- CS ' . $this->sidebox_filter_key . ' searchbox start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' searchbox end -->/s',
			                                                              'replace_string' => '{include file="file:' . $template_file . '"}' );
		}
		/* Search Functions */
...
*/
			...
			$sql = "SELECT * FROM booking WHERE title LIKE '%{$searchKeywords}%' OR description LIKE '%{$searchKeywords}%' ";
