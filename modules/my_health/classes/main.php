...
class my_health_module extends PageListModule {
    ...
    public $currentCategoryImage;
    public $currentCategoryIconImage;
    ...
    function __construct($module_path, $paginate, $paginate_items_per_page, $pageUrl, $module_key) {
        ...
        $this->pluralName = 'Articles';
        $this->singleName = 'Article';
        $this->section_page_name = 'My Health';

        // technical names
        $this->table = 'my_health';
        $this->categoryTable = 'my_health_category';
        $this->linkTable = 'my_health_category_lookup';
        ...
        $this->currentCategoryIconImage = '';

        /* #module specific */
        $this->model = new my_healthmodel($pageUrl);
        $this->model->setTable('my_health');

        $this->model->setpublishedField('published');
        $this->model->setOrderField('order_num desc');

        /* #module specific */
        $this->sidebox_filter_key = "my_health";
...
WHERE page_type.config_flag = '" . $module_key . "';";
    ...
    function DisplayItem($id) {
        ...
        $_SESSION['my_health_article_title'] = $item['title'];
        ...
        // slapout the form here 
        $form_id = MY_HEALTH_INLINE_FORM_ID;
    ...
    function SetCategoryForPage($pagename) {
        ...
        $row = db_get_single_row("select * from {$this->categoryTable} where page_name = '$pagename'");
        $this->currentCategory = $row['id']; 
        
        $this->currentCategoryImage = $row['page_image']; 
        $this->currentCategoryIconImage = $row['icon']; 
    ...
    function GetCategoryTitle() {
     ...
     function DisplayFeatureList(){
        ...
        // override specifically for this module 
        ...
        */
        ...
        $sql = " SELECT distinct mh.*, mhc.icon FROM my_health mh
 INNER JOIN my_health_category_lookup mhclu ON mhclu.`item_id` = mh.id 
 INNER JOIN my_health_category mhc ON mhclu.`category_id` = mhc.`id`
  WHERE published = 1 AND featured =1  
  ORDER BY mh.order_num  ";
      //    echo $sql;
    ...
    function DisplayList() {
        ...
        $smarty->assign('categoryImage', $this->currentCategoryImage);
        $smarty->assign('categoryIconImage', $this->currentCategoryIconImage);
    ...
    function ReadItems($sql) {
        ...
        while ($row = mysql_fetch_array($result)) {
            ...
            $items[] = array(
                ...
                'thumb' => $row['thumb'],
                 'icon' => $row['icon'], // specific my_health
            );
        };
    ...
    function GetItems() {
        ...
        if ($this->currentCategory == '') {
            ...
            $sql = "SELECT SQL_CALC_FOUND_ROWS t.id, title, description, thumb, description, page_name " .
                  ...
                  /*    $sql = " SELECT mh.*, mhc.icon FROM my_health mh
 INNER JOIN my_health_category_lookup mhclu ON mhclu.`item_id` = mh.id 
 INNER JOIN my_health_category mhc ON mhclu.`category_id` = mhc.`id`
  WHERE published = 1    AND mhclu.category_id= {$this->currentCategory}
ORDER BY mh.order_num DESC " ;
*/

            $sql = "SELECT `my_health`.* , my_health_category.icon,my_health_category.page_image  FROM `my_health`
LEFT JOIN my_health_category ON my_health_category.id = `my_health`.category_id
WHERE category_id = {$this->currentCategory} ORDER BY `my_health`.order_num  ";
   ...
   //     echo $limits;
    //    $sql .= $limits;
       ...
       // echo 'current cat = ' . $this->currentCategory;
      //  echo $sql;
    ...
    function createSearchBox($hasCategories) {
        ...
        global $module_path;
        global $module_prefix;
        $template_file = "$base_path/modules/$module_path/templates/ajax_search.tpl";
        $smarty->assign('hasCategories', $hasCategories);
        $this->AssignSearchCategories();
        
        $filters[$this->itemName . '_my_health_search_filter'] = array('search_string' => '/<!-- CS ' . $this->sidebox_filter_key . ' searchbox start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' searchbox end -->/s',
            'replace_string' => '{include file="file:' . $template_file . '"}');
    }
