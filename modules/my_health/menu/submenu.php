...
if(MY_HEALTH_ON_LEVEL == 0){
    ...
    /* #module specific */
    $submenu = get_my_health_Submenu($menulink);
...
/* #module specific */
function get_my_health_Submenu($menulink) {
    // change trigger
    /* #module specific */
  if(MY_HEALTH_HAS_CATEGORIES != 0){
	...
	$pageMenu = array();	
        ...
        /* #module specific */
	/*	$subnav_sql="SELECT DISTINCT c.* FROM my_health_category_lookup csc
INNER JOIN my_health cs ON cs.id = csc.item_id
INNER JOIN my_health_category c ON c.id = csc.category_id
WHERE cs.`published` = 1 AND special = 0 
ORDER BY c.order_num";
        */
      $subnav_sql="SELECT  DISTINCT my_health_category.* FROM `my_health` LEFT JOIN my_health_category ON my_health_category.id = `my_health`.category_id  ";
