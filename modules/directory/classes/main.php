...
class DirectoryModule extends PageListModule {
    ...
    function __construct($module_path, $paginate, $paginate_items_per_page, $pageUrl, $module_key) {
        ...
        $this->listName = 'Members';
        $this->itemName = 'directory';
        $this->pluralName = 'Directories';
        $this->singleName = 'Directory';
        $this->section_page_name = 'Directory';

        // technical names
        $this->table = 'shop_member_user';
        $this->categoryTable = 'directory_category';
        $this->linkTable = 'directory_category_lookup';
        $this->linkField = 'category_id';
        $this->keyField = 'item_id';

        $this->sideboxListName = 'sideboxdirectories'; // ?? odd name
        $this->pageName = $pageName;
        $this->currentCategory = "";

        /* #module specific */
        $this->model = new directorymodel($pageUrl);
        $this->model->setTable('directory');

        $this->model->setpublishedField('published');
        $this->model->setOrderField('order_num desc');

        /* #module specific */
        $this->sidebox_filter_key = "directory";
...
WHERE page_type.config_flag = '" . $module_key . "';";
    ...
    function DisplayList() {
        ...
        $alpha_template = $this->moduleFolder . 'templates/alpha.tpl';
        $smarty->assign('alpha_template', $alpha_template);
    ...
    function createSearchBox($hasCategories) {
        ...
        $filters[$this->itemName . '_directory_search_filter'] = array('search_string' => '/<!-- CS ' . $this->sidebox_filter_key . ' searchbox start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' searchbox end -->/s',
            'replace_string' => '{include file="file:' . $template_file . '"}');
    }

    /* Search Functions */
    function DisplaySearchResults($searchKeywords, $category, $alpha) {
        global $smarty;
        global $currentPageUrl;


        $items = $this->GetSearchResultItems($searchKeywords, $category, $alpha);

       /* todo: fix the pagination, limits but doesn't pass on search query */
        if ($this->paginate) {
            $paginationStr = $this->pagination->html_string($currentPageUrl . '?keywords=' . $_GET['keywords'] . '&alpha=' . $_GET['alpha'] . '&page=' );
            $smarty->assign('pagination', $paginationStr);
        }
    ...
    function GetSearchResultItems($searchKeywords, $category, $alpha) {
        ...
        if (!empty($searchKeywords)) {
            $match = "MATCH (t.firstname, t.surname) AGAINST ('$searchKeywords' IN BOOLEAN MODE)";
        } else {
            $match = ' 1 ';
        }
        //$sql = "select  SQL_CALC_FOUND_ROWS t.*, $match as score from {$this->table} t $cat_join where $cat_clause t.status = 1 and $match"; //  . $limits;  // re introduce when pag is working
        // $paginationStr = $this->pagination->html_string($currentPageUrl . '?page=');

        if (!empty($searchKeywords)) {
            $sql = "SELECT SQL_CALC_FOUND_ROWS t.*, t.* FROM shop_member_user t WHERE firstname LIKE '%$searchKeywords%' OR surname LIKE '%$searchKeywords%'  order by surname " . $limits;
        }else{
            // this should be alpha
            $sql = "SELECT SQL_CALC_FOUND_ROWS t.*, t.* FROM shop_member_user t WHERE surname LIKE '$alpha%' order by surname " . $limits;
        }
