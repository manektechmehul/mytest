...
class ProfilesModule extends PageListModule {
    ...
    function __construct($module_path, $paginate, $paginate_items_per_page, $pageUrl, $module_key) {
        ...
        $this->listName = 'profiles';
        $this->itemName = 'profile';
        $this->pluralName = 'Profiles';
        $this->singleName = 'Profile';
        $this->section_page_name = 'Profiles';

        // technical names
        $this->table = 'profiles';
        $this->categoryTable = 'profiles_category';
        $this->linkTable = 'profiles_category_lookup';
        $this->linkField = 'category_id';
        $this->keyField = 'item_id';

        $this->sideboxListName = 'sideboxprofiles'; // ?? odd name
        $this->pageName = $pageName;
        $this->currentCategory = "";

        /* #module specific */
        $this->model = new profilesmodel($pageUrl);
        ...
        /* #module specific */
        $this->sidebox_filter_key = "profiles";
...
WHERE page_type.config_flag = '" . $module_key . "';";
    ...
    function GetOGData($page, $thumb) {
        ...
        // this modules uses the title as part of the name
        $og = array(
            'title' => $data['title'] . ' ' .  $data['firstname'] . ' ' .  $data['surname'],
            'thumb' => '/UserFiles/Image/' . $data['thumb'],
            'description' => truncate(trim(strip_tags($data['description'])),157)
        );
        return $og;
    }

    function GetTitle($page_name) {
        // overriding the standard title with the persons details
        
           if(!is_int($id) ? (ctype_digit(id)) : true ){ 
                $sql = "select CONCAT_WS(' ',Title,firstname,surname) as title from {$this->table} where id = '$page_name'";
            }else{                    
                $sql = "select CONCAT_WS(' ',Title,firstname,surname) as title from {$this->table} where page_name = '$page_name'";     
            }          
    ...
    function DisplayItem($id) {
        ...
        $item_sql = "SELECT p.id, p.title, description, thumb, page_image, published, order_num, page_name, featured, firstname, surname, gender_id, age_group_id, phone, email, specialist_topic, organisation, organisation_position, website_social_link, pg.title AS gender , pag.title AS age_group 
FROM profiles p 
INNER JOIN profiles_gender pg ON pg.id = p.gender_id 
INNER JOIN profiles_age_group pag ON pag.id = p.age_group_id 
WHERE page_name = '{$id}'";
    ...
    function GetCategoryTitle() {
     ...
     function DisplaySinglePageCategoryList() {
        ...
        $sql = "SELECT  p.*, pc.title AS category_title FROM profiles p
INNER JOIN profiles_category pc ON pc.id = p.profiles_category
WHERE published = 1 
ORDER BY pc.title, p.surname";
        ...
        $smarty->assign('managements', $management);
        $smarty->assign('receptions', $reception);
        ...
        $templateFile = $this->moduleFolder . 'templates/singlePageCategoryList.tpl';
        $smarty->display("file:" . $templateFile);
    }
    ...
    function GetItems() {
        ...
        if ($this->currentCategory == '') {
            $cat_clause = '';
            /* #module specific - addfields */
            $sql = "SELECT SQL_CALC_FOUND_ROWS t.id, title,firstname, surname, description, thumb, description, page_name " .
                    " from {$this->table} t " .
                    "where published = 1 " .
                    //"and deadline > now() ".
    ...
    function createSearchBox($hasCategories) {
        ...
        $filters[$this->itemName . '_profiles_search_filter'] = array('search_string' => '/<!-- CS ' . $this->sidebox_filter_key . ' searchbox start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' searchbox end -->/s',
            'replace_string' => '{include file="file:' . $template_file . '"}');
    }
