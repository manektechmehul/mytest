...
class CaseStudiesModule extends PageListModule {
    ...
    function __construct($module_path, $paginate, $pageUrl, $module_key) {
        ...
        // Surely I can populate this lot from conf table
        $this->listName = 'casestudies';
        $this->itemName = 'casestudy';
        $this->pluralName = 'Case Studies';
        $this->singleName = 'Case Study';
        $this->section_page_name = 'casestudies';

        $this->table = 'case_study';
        $this->linkTable = 'case_study_category';
        $this->categoryTable = 'category';
        $this->linkField = 'category_id';
        $this->keyField = 'case_study_id';
        ...
        // CaseStudiesModel
        $this->model = new model($pageUrl);
        $this->model->setTable('case_study');
        $this->model->setpublishedField('published');
        $this->model->setOrderField('order_num desc');
               

        // new ones to put in the db
        $this->sidebox_filter_key = "case studies";
    ...
    function DisplayItem($id) {
     ...
     //   } else {
      ...
      //  }
       ...
       $factors_links = db_get_rows('SELECT 
f.title, f.page_name 
FROM `case_study_category` csc
INNER JOIN factor f ON f.id = csc.`category_id`
WHERE csc.case_study_id =' . $item['id']);
        
     //   var_dump($factors_links);
        
        $smarty->assign('factors_links',$factors_links);
    ...
    /*
    function DisplayFixedCategoryList(){
    ...
    */
    ...
    function DisplayList() {
        ...
        $smarty->assign('itemList', $items);
        // todo
        // $smarty->assign('module_root', '/project_profiles');
        if ($this->paginate) {
    ...
    function getFeaturedSidebox() {
        ...
        $sql = "select * from {$this->table} ";       
        // got rid of category inner join stuff.
        $sql .= " where published = 1 and featured =1 ";
