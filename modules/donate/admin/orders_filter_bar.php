...
<script>
 $(function() {
	...
	$( "#end_date" ).datepicker({
	 ...
	 var donation_type_val = <?
if(isset($_GET['donation_type'])){
	 echo  $_GET['donation_type'];
}else{
	 echo '-1'; //default to all
}
?>;
	 $("#donation_type").val(donation_type_val);
...
<table width="100%" class="adminfiltertable">
  <tr>
    ...
    <td align="left"><select name="status"  id="status" >
	  ...
	  <td class="adminfilter-right">Donation Type</td>
	  <td align="left"><select name="donation_type"  id="donation_type" >
			  <option value="-1"> All </option>
			  <? if(true){


				 $r = db_get_rows("SELECT id, title FROM donate WHERE published = 1 ORDER BY title ");
				//  var_dump($r);
				 // die();
