<?php
	...
	class orders extends template {
		function orders() {
			...
			$this->max_items       = - 1;
			$this->javascript_file = '/modules/donate/admin/js/orders.js';
			...
			$this->buttons = array(
				...
				'items' => array(
					...
					'type' => 'button',
					'pattern' => '/modules/donate/admin/order_items.php?order=%s'
				),
				'progress_order' => array(
					'text' => 'change status',
					'type' => 'button',
					'pattern' => '/modules/donate/admin/orders.php?change_status=%s'
				),
				'resend_email' => array(
					'text' => 'resend email',
					'type' => 'jsbutton',
		...
		function set_customer_id( $id, $row ) {
			...
			$href        = '/modules/donate/admin/orders.php?edit_customer=' . $customer_id;
		...
		function get_customer_filter_bar_custom_sql() {
			$where = " where soi.`product_type` = 1   "; // only every show the donate entries
