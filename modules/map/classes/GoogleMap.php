...
class GoogleMapAPI {
    ...
    function setMarkerIconKey($iconImage,$iconShadow='',$iconAnchorX = 'x',$iconAnchorY = 'x',$infoWindowAnchorX = 'x',$infoWindowAnchorY = 'x'){
    	...
    	if(isset($this->_marker_icons[$_iconKey])){
            ...
            // to offset the icon with the pin in the bottom centre, set the first number to half the icon image width and the second to a 4px over the height.
    		return $this->addIcon($iconImage,$iconShadow,14.5,42,$infoWindowAnchorX,$infoWindowAnchorY);
    ...
    function getHeaderJS() { 
		...
		if(!empty($this->_elevation_polylines)||(!empty($this->_directions)&&$this->elevation_directions)){
			...
			<script type='text/javascript'>
                ...
                //GL: I taked on the places library for autocomplete
        $scriptUrl = "http://maps.google.com/maps/api/js?sensor=".(($this->mobile==true)?"true":"false") . '&libraries=places';
