...
if(VIDEO_ON_LEVEL == 0){
    ...
    /* #module specific */
    $submenu = getVideosSubmenu($menulink);
...
/* #module specific */
function getVideosSubmenu($menulink) {
    // change trigger
    /* #module specific */
  if(VIDEO_HAS_CATEGORIES != 0){
	...
	$pageMenu = array();	
        ...
        /* #module specific */
		$subnav_sql="SELECT DISTINCT c.* FROM video_category_lookup csc
INNER JOIN video cs ON cs.id = csc.item_id
INNER JOIN video_category c ON c.id = csc.category_id
