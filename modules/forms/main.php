...
class contact_form extends form_template {
    ...
    function process_data() {
        ...
        if (!$this->no_enquiry)
            // standard_email
			echo 'starting mail encryption';
			
            $this->encrypted_standard_email($site_address, $to_email_address, $from_email_address, $subject);
    ...
    function save_data() {
        $data = $this->data;
        $sql = sprintf("replace into registrants (firstname, surname, emailaddress, receiveemail, added) values ('%s', '%s', '%s', '%s', '%s') ", $data['firstname'], $data['surname'], $data['emailaddress'], $data['receiveemail'], date('Y-m-d'));
