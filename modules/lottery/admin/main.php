<?php
	...
	class orders extends template {
		function orders() {
			...
			$this->max_items       = - 1;
			$this->javascript_file = '/modules/lottery/admin/js/orders.js';
			...
			$this->buttons = array(
				...
				'items' => array(
					...
					'type' => 'button',
					'pattern' => '/modules/lottery/admin/order_items.php?order=%s'
				),
				'progress_order' => array(
					'text' => 'change status',
					'type' => 'button',
					'pattern' => '/modules/lottery/admin/main.php?change_status=%s'
				),
				...
				'resend_email' => array(
					...
					'type' => 'jsbutton',
					'pattern' => '#',
					'js' => "onclick='return resend_email(%s)'"
				),

				'related' => array( 'name' => 'Related_Sales', 'type' => 'function' , 'function' => 'related_items' ),
			...
			$this->actions = array(
				'change_status' => array(
					'title' => 'Change Status',
					'pagequerystring' => 'change_status',
					'pagemethod' => 'change_status',
					'actionquerystring' => 'process_change_status',
					'actionmethod' => 'process_change_status'
				),
				'edit_customer' => array(
					'title' => 'Edit Customer Details',
					'pagequerystring' => 'edit_customer',
					'pagemethod' => 'edit_customer',
					'actionquerystring' => 'process_customer',
					'actionmethod' => 'process_customer'
				),
			);
		}

		function related_items($id){
			// go to a page to list other items with the same futurePayId

            // check if this one is a has a futurepayID
			$sql        = "SELECT soil.* FROM  shop_order_item soi
  INNER JOIN shop_order_item_lottery soil ON soil.`shop_order_item_id` = soi.`id` WHERE soi.product_type = 3 AND soi.product_id = 2  AND soi.order_id =   '$id'";
		 	$row = db_get_single_row( $sql );

			if($row['futurePayId']) {
				$statusName = "related recurring orders";
				$href       = "related_recurring.php?fpid=" . $row['futurePayId'];
		...
		function get_customer_filter_bar_custom_sql() {
			...
			$where = " where soi.`product_type` = 3   "; // only every show the lottery entries
			if ( $_GET['payment_type'] != '-1' && $_GET['payment_type'] != '' ) {
				$where .= " and soil.subscription =" . $_GET['payment_type'] . " ";
			}
		/*	if ( $_GET['status'] != '-1' && $_GET['status'] != '' ) {
				$where .= " and shop_order.status=" . $_GET['status'] . " ";
			}
		*/
			...
			if ( $_GET['id'] != '' ) {
				$where .= " and  shop_order.id=" . $_GET['id'] . " ";
			}
			if ( $_GET['start_date'] != '' ) {
				$where .= " and time_made > '" . date( "Y-m-d", strtotime( $_GET['start_date'] ) ) . "' ";
			}
			if ( $_GET['end_date'] != '' ) {
				$where .= " and time_made < '" . date( "Y-m-d", strtotime( $_GET['end_date'] ) ) . "' ";
			}
			if ( $_GET['email'] != '' ) {
				$where .= " and email = '" . $_GET['email'] . "' ";
			}
			if ( $_GET['firstname'] != '' ) {
				$where .= " and firstname = '" . $_GET['firstname'] . "' ";
			}
			if ( $_GET['surname'] != '' ) {
				$where .= " and surname = '" . $_GET['surname'] . "' ";
			}
			...
			$sql = "SELECT distinct shop_customer.firstname, shop_customer.surname, shop_order.*, unix_timestamp(shop_order.time_made) as time_made_formated
FROM shop_order
INNER JOIN shop_customer ON shop_customer.`id` = shop_order.`customer_id`
 JOIN shop_order_item soi
    ON shop_order.`id` = soi.`order_id`
 JOIN shop_order_item_lottery soil
