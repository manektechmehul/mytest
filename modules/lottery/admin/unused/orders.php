...
class orders extends template {
	...
	function orders() {
		...
		$this->buttons = array(
			...
			'items' => array('text' => 'edit order items', 'type' => 'button', 'pattern' => '/modules/lottery/admin/order_items.php?order=%s'),
			'progress_order' => array('text' => 'change status', 'type' => 'button', 'pattern' => '/modules/lottery/admin/main.php?change_status=%s'),
	...
	function get_customer_filter_bar_custom_sql(){
		...
		if(isset($_GET['order_by'])){
			...
			$order = " ORDER BY shop_order.status, time_made desc ";
		}
		...
		$sql = "SELECT shop_customer.firstname, shop_customer.surname, shop_order.*, unix_timestamp(shop_order.time_made) as time_made_formated
FROM shop_order
INNER JOIN shop_customer ON shop_customer.`id` = shop_order.`customer_id`
