{section name=item loop=$itemList}
    <div class="{$listName}left">
        ...
        {show_thumb filename=$itemList[item].thumb size='60x60' alt=$alt border="0" class="class=left"'}
        
        
      <a href="/{$page_url_parts[0]}/{$itemList[item].page_name}">{show_thumb filename=$itemList[item].page_image size='180x600' alt=$alt border="0" class="class=left"'}</a>
    </div>
