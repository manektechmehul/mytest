...
if(FACTOR_ON_LEVEL == 0){
    ...
    /* #module specific */
    $submenu = getFactorsSubmenu($menulink);
...
/* #module specific */
function getFactorsSubmenu($menulink) {
    // change trigger
    /* #module specific */
  if(FACTOR_HAS_CATEGORIES != 0){
	...
	$pageMenu = array();	
        ...
        /* #module specific */
		$subnav_sql="SELECT DISTINCT c.* FROM factor_category_lookup csc
INNER JOIN factor cs ON cs.id = csc.item_id
INNER JOIN factor_category c ON c.id = csc.category_id
