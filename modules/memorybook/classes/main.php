<?php
	...
	class MemorybookModule extends PageListModule {
		public $listName;
		...
		function __construct( $module_path, $paginate, $paginate_items_per_page, $pageUrl, $module_key ) {
			...
			$this->listName          = 'memories';
			$this->itemName          = 'memorybook';
			$this->pluralName        = 'Memories';
			$this->singleName        = 'Memorybook';
			$this->section_page_name = 'Memorybook';
			// technical names
			$this->table         = 'memorybook';
			$this->categoryTable = 'memorybook_category';
			$this->linkTable     = 'memorybook_category_lookup';
			$this->linkField     = 'category_id';
			$this->keyField      = 'item_id';
			$this->sideboxListName = 'sideboxmemories'; // ?? odd name
			$this->pageName        = $pageName;
			$this->currentCategory = "";
			/* #module specific */
			$this->model = new memorybookmodel( $pageUrl );
			$this->model->setTable( 'memorybook' );
			$this->model->setpublishedField( 'published' );
			$this->model->setOrderField( 'order_num desc' );
			/* #module specific */
			$this->sidebox_filter_key = "memorybook";
...
WHERE page_type.config_flag = '" . $module_key . "';";
		...
		function DisplayItem( $id ) {
			global $smarty;
			global $name_parts;
			$smarty->assign( 'button_text', 'Back to ' . $this->pluralName . ' search' );
		...
		function getLatestSidebox() {
			...
			$filters[$this->itemName . '_latest_sidebox_filter'] = array(
				'search_string' => '/<!-- CS ' . $this->sidebox_filter_key . ' latest start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' latest end -->/s',
		...
		function getFeaturedSidebox() {
			...
			$filters[$this->itemName . '_featured_sidebox_filter'] = array(
				'search_string' => '/<!-- CS ' . $this->sidebox_filter_key . ' featured start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' featured end -->/s',
				'replace_string' => '{include file="file:' . $template_file . '"}'
			);
		}
		function createSearchBox( $hasCategories=false ) {
			global $smarty;
			global $base_path;
			global $filters;
			global $module_path;
			...
			$filters[$this->itemName . '_search_filter'] = array(
				'search_string' => '/<!-- CS ' . $this->sidebox_filter_key . ' searchbox start -->(.*)<!-- CS ' . $this->sidebox_filter_key . ' searchbox end -->/s',
				'replace_string' => '{include file="file:' . $template_file . '"}'
			);
		}
		/* Search Functions */
		function DisplaySearchResults( $searchKeywords, $category ) {
			global $smarty,$currentPageUrl, $name_parts;
		   $page_params = "?";
			foreach ($_GET as $k => $v) {
				if($k != 'page') {
					$page_params .= $k . '=' . $v . '&';
			...
			/* Found Exact Match - so Jump to it */
			if(sizeof($items) == 1){
				$url = "/" . $name_parts[0] . "/" . $items[0]['page_name'];
			...
			/* todo: fix the pagination, limits but doesn't pass on search query */
				if ($this->paginate) {
			 		$paginationStr = $this->pagination->html_string($currentPageUrl . $page_params . 'page=' );
					$smarty->assign('pagination',  $paginationStr);
				}
		...
		function getMemories($pagename){


			$sql = "SELECT  mbm.id, mbm.`name`,mbm.`email`, mbm.`memory`, DATE_FORMAT(mbm.`date`,'%d/%m/%Y') as date FROM memorybook mb
INNER JOIN memorybook_memory mbm ON mbm.`memorybook_id` = mb.id
WHERE page_name = '{$pagename}' AND mbm.published = 1";
		...
		function add_memory( $name, $memory, $email, $pagename ) {

			$name = mysql_real_escape_string($name);
			$memory = mysql_real_escape_string($memory);
			$email = mysql_real_escape_string($email);

			$memorybook_sql = "select id from memorybook where page_name = '{$pagename}'";
			$memorybook_id = db_get_single_value( $memorybook_sql );
			// published defaults to 0
			$sql = " insert into memorybook_memory (`name`, `memory`, email,  `memorybook_id`) values('{$name}','{$memory}','{$email}','{$memorybook_id}'); ";

			$result = mysql_query($sql);
			if ( $result ) {
				// send admin an email to say there is one to check and set live

				$this->SendConfirmationEmail($memorybook_id);
		...
		function SendConfirmationEmail($memorybook_id) {
...
/*
			...
			*/
			...
			$patient_name = db_get_single_value("select title from memorybook where id ='{$memorybook_id}' ");

			// echo $patient_name;
			// die();
			$smarty->assign('patient_name', $patient_name);

			$message['to'] = SITE_CONTACT_EMAIL; // replace this with the person who will validate the messages
			// $message['bcc'] = SITE_CONTACT_EMAIL;
			// echo ' site contact email is ' .  SITE_CONTACT_EMAIL;
			// die();
			$message['body'] = $smarty->fetch($base_path . '/modules/memorybook/templates/confirmation_email.tpl');
			$message['subject'] = "New Memory Book Entry";
		...
		function SendEmail($message) {
			...
			//echo "did the mail send ??? " . $mail->Send();
			//var_dump($mail);
		}
