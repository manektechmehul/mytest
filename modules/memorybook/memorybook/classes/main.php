...
WHERE page_type.config_flag = '" . $module_key . "';";
		...
		function GetSearchResultItems( $searchKeywords, $category ) {
		 ...
		 /*   $sql = "select  SQL_CALC_FO_ROWS t.*, $match as score
			...
			// $paginationStr = $this->pagination->html_string($currentPageUrl . '?page=');
			 echo $sql;
		   */
			$sql = "SELECT * FROM memorybook WHERE title LIKE '%$searchKeywords%' ";
			$items = $this->ReadItems( $sql );
			/*
				if ($this->paginate) {
					$this->pagination->set_pages();
		...
		function getMemories($pagename){
			...
			$sql = "SELECT  mbm.id, mbm.`name`,mbm.`email`, mbm.`memory` FROM memorybook mb
