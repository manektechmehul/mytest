...
if(MEMORYBOOK_ON_LEVEL == 0){
    ...
    /* #module specific */
    $submenu = getMemoriesSubmenu($menulink);
...
/* #module specific */
function getMemoriesSubmenu($menulink) {
    // change trigger
    /* #module specific */
  if(MEMORYBOOK_HAS_CATEGORIES != 0){
	...
	$pageMenu = array();	
        ...
        /* #module specific */
		$subnav_sql="SELECT DISTINCT c.* FROM memorybook_category_lookup csc
INNER JOIN memorybook cs ON cs.id = csc.item_id
INNER JOIN memorybook_category c ON c.id = csc.category_id
