...
class main extends template {
    ...
    function main() {
        ...
        $this->buttons = array(
	       ...
	       // 'memories' => array('text' => 'memory posts', 'type' => 'button', 'pattern' => '/modules/memorybook/admin/memories.php?memorybook=%s'),

	        'memories' => array('type' => 'function', 'function' => 'memories_count'),
       ...
       //     'page_image' => array('name' => 'Page Image', 'formtype' => 'image'),
            ...
            'description' => array('name' => 'Nickname or nee', 'formtype' => 'text', 'list' => false, 'rows' => 3, 'cols' => 60, 'required' => true),
       ...
       //     'clear_custom_seo' => array('name' => 'Clear Custom SEO<br> (Check this box then submit)', 'formtype' => 'checkbox', 'not_field' => true),
            ...
            'date_of_birth' => array('name' => 'Date of Birth', 'formtype' => 'date', 'required' => true, 'start_year'=> '1900' ),
            'date_of_death' => array('name' => 'Date of Death', 'formtype' => 'date', 'required' => true ),
            'allow_comments' => array('name' => 'Allow Comments', 'formtype' => 'checkbox'),
            
        );
        // this will populate the link table - categories
         /* #module specific */
        $this->links = array('category' => array('link_table' => 'memorybook_category_lookup', 'table' => 'memorybook_category', 'name' => 'title', 'primary'=>'item_id', 'foreign'=>'category_id'));
    }


	function memories_count($id){

		$new_count = db_get_single_value("SELECT COUNT(*) FROM memorybook_memory WHERE  new = 1 and  memorybook_id={$id}");
		$href= "memories.php?memorybook=" . $id;
		$label = "memory posts";
		if($new_count > 0){
	...
	function onload() {
		...
		if(MEMORYBOOK_HAS_CATEGORIES){	
			$this->fields['category'] = array('name' => 'Categories', 'formtype' => 'checklink', 'required' => true, 'not_field' => true, 'link' => 'category','customfunction' => 'category_checklist');
		}
    ...
    function get_form_data() {
        $this->module_id = db_get_single_value("select id from module where constant = 'SITE_HAS_MEMORYBOOK'", "id");
   ...
   function category_checklist($id, $fieldname, $field) {
        ...
        /* #module specific */
        $linksql = "SELECT t.id, t.title, l.item_id FROM memorybook_category t 
LEFT OUTER JOIN memorybook_category_lookup l ON t.id = l.category_id AND l.item_id = '$id' order by title";      
