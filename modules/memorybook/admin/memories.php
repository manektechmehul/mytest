<?php
	...
	class posts extends template {
		...
		function posts() {
			$this->template();
			$this->override_cancel_button = true;
			$this->override_cancel_button_code = "<INPUT TYPE=\"button\" VALUE=\"Cancel\" onClick=\"window.location='memories.php?memorybook=". $_GET["memorybook"] . "'\">";
		...
		function onload() {
			//
			if(isset($_GET['id'])){
				db_update("update memorybook_memory set new = 0 where id =" .  $_GET['id'] );
			}
		}
