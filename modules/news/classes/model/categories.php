...
class CategoriesModel extends ListModel
{
	...
	{
		$parentId = db_get_single_value("select id from category where page_name = '$pageName'");
	...
	{
		$sql = "select * from category where page_name = '$pageName'";
	...
	{
		$parentId = db_get_single_value("select * from category where id = (select parent from category where id = '$id')");
		return db_get_single_value("select page_name from category where id = '$parentId'");

		
	}
	function GetSubmenuItems($id = 0)
	{
		$sql = "SELECT c.id, c.title, c.page_name FROM category c where parent = $id";
